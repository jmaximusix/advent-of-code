...
next :: Set.Set VecPos -> VecPos -> [(VecPos, Int)]
next nblocks = map (,1) . filter (\v'@(V2 a b) -> min a b >= 0 && max a b <= 70 && v' `Set.notMember` nblocks) . neighbors

blocks :: [String] -> [VecPos]
blocks = map (uncurry V2 . tup2 . readNumbers)

solveUntilN :: Int -> [VecPos] -> Maybe Int
solveUntilN n allblocks = fst <$> dijkstraAssoc (next nblocks) (== V2 70 70) (V2 0 0)
  where
